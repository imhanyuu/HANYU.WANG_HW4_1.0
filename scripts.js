...
function searchCity() {
    ...
    var cityBlocks = document.getElementById("city-buttons").getElementsByClassName("city-block");
    for (var i = 0; i < cityBlocks.length; i++) {
        var button = cityBlocks[i].getElementsByTagName("button")[0];
        if (button.innerHTML.toUpperCase().indexOf(filter) > -1) {
            cityBlocks[i].style.display = "";
        } else {
            cityBlocks[i].style.display = "none";
        }
    }
}
    ...
    sisterCities.forEach(city => {
        ...
        button.onclick = function() {
            ...
            new mapboxgl.Popup()
                ...
                .setHTML(`<h3>${city.city}</h3><p>${city.intro}</p>`)
                .addTo(map);
        };
        cityBlock.appendChild(button);

        const cityIntro = document.createElement('div');
        cityIntro.textContent = city.intro;
        cityIntro.className = 'city-intro';
        cityBlock.appendChild(cityIntro);

        cityButtons.appendChild(cityBlock);
